...
const GithubProfileItem = ({uid, login}) =>
    <li>
        <GithubProfileLink username={login}>
            <GithubAvatar uid={uid} username={login}/>
        </GithubProfileLink>
    </li>;
    ...
    render() {
        ...
        return (
            <InlineCss stylesheet={Main.css(avatarSize)} namespace="Main">
                ...
                <ul>
                    <li><a href={repositoryUrl} title="star = join us!">
                        <img className="avatar" src={avatarUrl(0)} alt="you?"/>
                    </a></li>

                    {stargazers.map(user =>
                        <GithubProfileItem key={user.id} uid={user.id} username={user.login}>
                        </GithubProfileItem>
                    )}
